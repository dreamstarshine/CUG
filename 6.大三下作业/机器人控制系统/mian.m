...
for j=1:MAX_Y
    for i=1:MAX_X
        if (MAX(i,j)==1)
            plot(i+.5,j+.5,'ks','MarkerFaceColor','b');
            n=n+1;
        end
    end
end
...
q_goal=[10,15];
% q_goal=[10,11];
plot(q_start(1)+.5,q_start(2)+.5,'go','MarkerFaceColor','g');
plot(q_goal(1)+.5,q_goal(2)+.5,'ro','MarkerFaceColor','r');
text(q_start(1)+.5,q_start(2)+.5,'起点');
text(q_goal(1)+.5,q_goal(2)+.5,'终点');
